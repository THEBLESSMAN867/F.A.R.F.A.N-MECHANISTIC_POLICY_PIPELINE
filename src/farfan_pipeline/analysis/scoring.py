...
class MicroQuestionScorer:
    ...
    @calibrated_method("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_a")
    def score_type_a(self, evidence: Evidence) -> tuple[float, dict[str, Any]]:
        ...
        - Threshold: get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_a").get("auto_param_L245_21", 0.7)
        - Max_score: 3
        - Expected_elements: 4
        ...
        3. Si ratio >= get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_a").get("auto_param_L252_23", 0.7): aplicar escala proporcional
        4. Si ratio < get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_a").get("auto_param_L253_22", 0.7): penalizar fuertemente
        ...
        - 0/4 elementos (0%) → get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_a").get("auto_param_L260_31", 0.0)
        ...
        # Calcular ratio
        ratio = elements_found / expected if expected > 0 else get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_a").get("auto_param_L274_63", 0.0)
        ...
        score = max(get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_a").get("auto_param_L285_20", 0.0), min(max_score, score))
    ...
    @calibrated_method("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_b")
    def score_type_b(self, evidence: Evidence) -> tuple[float, dict[str, Any]]:
        ...
        - 3+ elementos → 3.0
        - 2 elementos → 2.0
        - 1 elemento → get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_b").get("auto_param_L324_23", 1.0)
        - 0 elementos → get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_b").get("auto_param_L325_24", 0.0)
    ...
    @calibrated_method("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_c")
    def score_type_c(self, evidence: Evidence) -> tuple[float, dict[str, Any]]:
        ...
        - Aggregation: "presence_threshold"
        - Threshold: get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_c").get("auto_param_L366_21", 0.5)
        - Max_score: 3
        - Expected_elements: 2
        ...
        3. Si ratio >= get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_c").get("auto_param_L373_23", 0.5): aplicar escala proporcional
        4. Si ratio < get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_c").get("auto_param_L374_22", 0.5): penalizar

        ESCALA:
        - 2/2 elementos (100%) → 3.0
        - 1/2 elementos (50%) → 1.5
        - 0/2 elementos (0%) → get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_c").get("auto_param_L379_31", 0.0)
        ...
        # Calcular ratio
        ratio = elements_found / expected if expected > 0 else get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_c").get("auto_param_L393_63", 0.0)
        ...
        # Clip al rango [0, max_score]
        score = max(get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_c").get("auto_param_L404_20", 0.0), min(max_score, score))
    ...
    @calibrated_method("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d")
    def score_type_d(self, evidence: Evidence) -> tuple[float, dict[str, Any]]:
        """
        MÉTODO 4: TYPE_D - Count 3 elements, weighted [get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L428_55", 0.4), get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L428_60", 0.3), get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L428_65", 0.3)].

        ESPECIFICACIÓN (línea 34586 del monolith):
        - Aggregation: "weighted_sum"
        - Weights: [get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L432_20", 0.4), get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L432_25", 0.3), get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L432_30", 0.3)]
        - Max_score: 3
        - Expected_elements: 3

        LÓGICA:
        1. Se esperan 3 elementos con importancia diferente
        2. Elemento 1: peso get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L438_28", 0.4) (más importante)
        3. Elemento 2: peso get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L439_28", 0.3)
        4. Elemento 3: peso get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L440_28", 0.3)
        5. Score = (sum of weights for found elements) * max_score

        ESCALA:
        - 3 elementos (todos) → weights_sum=get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L444_44", 1.0) → 3.0
        - 2 elementos (ej: elem1+elem2) → weights_sum=get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L445_54", 0.7) → 2.1
        - 1 elemento (ej: elem1) → weights_sum=get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L446_47", 0.4) → 1.2
        - 0 elementos → get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L447_24", 0.0)
        ...
        # Clip al rango [0, max_score]
        score = max(get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_d").get("auto_param_L479_20", 0.0), min(max_score, score))
    ...
    @calibrated_method("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_e")
    def score_type_e(self, evidence: Evidence) -> tuple[float, dict[str, Any]]:
        ...
        2. Si existe: 3.0
        3. Si no existe: get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_e").get("auto_param_L511_25", 0.0)

        ESCALA:
        - Evidencia presente → 3.0
        - Evidencia ausente → get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_e").get("auto_param_L515_30", 0.0)
        ...
        has_evidence = (
            ...
            (evidence.semantic_similarity is not None and evidence.semantic_similarity > get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_e").get("auto_param_L529_89", 0.5))
        )

        # Binary: todo o nada
        score = max_score if has_evidence else get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_e").get("auto_param_L533_47", 0.0)
        ...
        details = {
            ...
            'raw_score': score,
            'formula': 'max_score if has_evidence else get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_e").get("auto_param_L542_55", 0.0)'
        }
    ...
    @calibrated_method("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_f")
    def score_type_f(self, evidence: Evidence) -> tuple[float, dict[str, Any]]:
        ...
        ESCALA:
        - Similarity = get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_f").get("auto_param_L569_23", 1.0) → 3.0
        - Similarity = get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_f").get("auto_param_L570_23", 0.75) → 2.25
        - Similarity = get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_f").get("auto_param_L571_23", 0.5) → 1.5
        - Similarity = get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_f").get("auto_param_L572_23", 0.25) → get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_f").get("auto_param_L572_30", 0.75)
        - Similarity = get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_f").get("auto_param_L573_23", 0.0) → get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_f").get("auto_param_L573_29", 0.0)
        ...
        # Fallback: calcular promedio de confidence_scores
            ...
            similarity = get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_f").get("similarity", 0.0) # Refactored

        # Normalización minmax (ya está en rango 0-1)
        normalized_similarity = max(get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_f").get("auto_param_L593_36", 0.0), min(get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.score_type_f").get("auto_param_L593_45", 1.0), similarity))
    ...
    @calibrated_method("farfan_pipeline.analysis.scoring.MicroQuestionScorer.determine_quality_level")
    def determine_quality_level(self, normalized_score: float) -> tuple[QualityLevel, str]:
        ...
        - EXCELENTE: ≥ get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.determine_quality_level").get("auto_param_L695_23", 0.85) (verde)
        - BUENO: ≥ get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.determine_quality_level").get("auto_param_L696_19", 0.70) (azul)
        - ACEPTABLE: ≥ get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.determine_quality_level").get("auto_param_L697_23", 0.55) (amarillo)
        - INSUFICIENTE: < get_parameter_loader().get("farfan_pipeline.analysis.scoring.MicroQuestionScorer.determine_quality_level").get("auto_param_L698_26", 0.55) (rojo)
