...
class BayesianConfidenceCalculator:
    ...
    def calculate_posterior(
            ...
            observations: int,
            domain_weight: float = get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.BayesianConfidenceCalculator.__init__").get("auto_param_L121_35", 1.0)
    ) -> float:
        """
        ...
        Args:
            evidence_strength: Strength of the evidence (get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.BayesianConfidenceCalculator.__init__").get("auto_param_L130_57", 0.0)-get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.BayesianConfidenceCalculator.__init__").get("auto_param_L130_61", 1.0) scale, unitless ratio)
            observations: Number of observations supporting the evidence (count)
            domain_weight: Policy domain-specific weight (multiplier, default: get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.BayesianConfidenceCalculator.__init__").get("auto_param_L132_79", 1.0))

        Returns:
            float: Posterior probability (get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.BayesianConfidenceCalculator.__init__").get("auto_param_L135_42", 0.0)-get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.BayesianConfidenceCalculator.__init__").get("auto_param_L135_46", 1.0) scale) representing confidence level
        """
        ...
        credible_interval = beta.interval(get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.BayesianConfidenceCalculator.__init__").get("auto_param_L145_42", 0.95), alpha_post, beta_post)

        # Adjust for uncertainty (wider intervals reduce confidence)
        uncertainty_penalty = get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.BayesianConfidenceCalculator.__init__").get("auto_param_L148_30", 1.0) - (credible_interval[1] - credible_interval[0])

        return min(get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.BayesianConfidenceCalculator.__init__").get("auto_param_L150_19", 1.0), posterior_mean * uncertainty_penalty)
...
class TemporalLogicVerifier:
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.TemporalLogicVerifier._build_timeline")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.TemporalLogicVerifier._parse_temporal_marker")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.TemporalLogicVerifier._has_temporal_conflict")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.TemporalLogicVerifier._extract_resources")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.TemporalLogicVerifier._check_deadline_constraints")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.TemporalLogicVerifier._should_precede")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.TemporalLogicVerifier._classify_temporal_type")
...
class PolicyContradictionDetector:
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._initialize_pdm_patterns")
    ...
    def detect(
        ...
        return {
            ...
            "high_severity_count": sum(1 for c in contradictions if c.severity > get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._initialize_pdm_patterns").get("auto_param_L469_81", 0.7)),
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph")
        ...
        for i, stmt in enumerate(statements):
            ...
            for j, other in enumerate(statements):
                if i != j:
                    ...
                    if similarity > get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L562_36", 0.7):  # Umbral de relación
        ...
        for i, stmt_a in enumerate(statements):
            for stmt_b in statements[i + 1:]:
                if stmt_a.embedding is not None and stmt_b.embedding is not None:
                    ...
                    if contradiction_score > get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L587_45", 0.7) and similarity > get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L587_66", 0.5):
                    ...
                    for claim_a in stmt_a.quantitative_claims:
                        for claim_b in stmt_b.quantitative_claims:
                            if self._are_comparable_claims(claim_a, claim_b):
                                ...
                                if divergence is not None and divergence > get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L632_75", 0.2):
                                    ...
                                    if p_value < get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L639_49", 0.05):  # Significancia estadística
                                        confidence = self.bayesian_calculator.calculate_posterior(
                                        ...
                                        evidence = ContradictionEvidence(
                                            ...
                                            confidence=confidence,
                                            severity=min(get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L651_57", 1.0), divergence),
                                            semantic_similarity=get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L652_64", 0.0),
    ...
    def _detect_temporal_conflicts(
        ...
        if len(temporal_statements) >= 2:
            ...
            for conflict in conflicts:
                ...
                confidence = self.bayesian_calculator.calculate_posterior(
                    evidence_strength=get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L686_38", 0.9),  # Alta confianza en lógica temporal
                ...
                evidence = ContradictionEvidence(
                    ...
                    confidence=confidence,
                    severity=get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L696_29", 0.8),  # Los conflictos temporales son severos
                    semantic_similarity=self._calculate_similarity(stmt_a, stmt_b),
                    logical_conflict_score=get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L698_43", 1.0),
            ...
            for cycle in negative_cycles:
                ...
                for i in range(len(cycle_statements)):
                    ...
                    if self._has_logical_conflict(stmt_a, stmt_b):
                        confidence = self.bayesian_calculator.calculate_posterior(
                            evidence_strength=get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L736_46", 0.85),
                            observations=len(cycle),
                            domain_weight = get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("domain_weight", 1.0) # Refactored
                        )
                        ...
                        evidence = ContradictionEvidence(
                            ...
                            confidence=confidence,
                            severity=get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L746_37", 0.7),
                            semantic_similarity=self._calculate_similarity(stmt_a, stmt_b),
                            logical_conflict_score=get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L748_51", 0.9),
                            temporal_consistency=True,
                            numerical_divergence=None,
                            affected_dimensions=[stmt_a.dimension, stmt_b.dimension],
                            resolution_suggestions=self._suggest_resolutions(
                    ...
                    for stmt_b, amount_b in allocations[i + 1:]:
                        if amount_a and amount_b:
                            if self._are_conflicting_allocations(
                                ...
                                confidence = self.bayesian_calculator.calculate_posterior(
                                    evidence_strength=get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L794_54", 0.8),
                                ...
                                evidence = ContradictionEvidence(
                                    ...
                                    confidence=confidence,
                                    severity=get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L804_45", 0.9),  # Conflictos de recursos son críticos
                                    semantic_similarity=self._calculate_similarity(stmt_a, stmt_b),
                                    logical_conflict_score=get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L806_59", 0.8),
                                    temporal_consistency=True,
    ...
    def _calculate_coherence_metrics(
        ...
        weights = np.array([get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L845_28", 0.3), get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L845_33", 0.25), get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L845_39", 0.2), get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L845_44", 0.15), get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L845_50", 0.1)])
        scores = np.array([
        ...
        coherence_score = np.sum(weights) / np.sum(weights / np.maximum(scores, get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L855_80", 0.01)))
    ...
    def _calculate_global_semantic_coherence(
        ...
        if len(statements) < 2:
            return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L881_19", 1.0)

        # Calcular matriz de similitud
        embeddings = [s.embedding for s in statements if s.embedding is not None]
        if len(embeddings) < 2:
            return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L886_19", 0.5)
        ...
        coherence = mean_similarity * (1 - min(get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L899_47", 0.5), std_similarity))
    ...
    def _calculate_objective_alignment(
        ...
        if len(objective_statements) < 2:
            return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L914_19", 1.0)
        ...
        return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._build_knowledge_graph").get("auto_param_L927_15", 0.5)

    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._calculate_graph_fragmentation")
    def _calculate_graph_fragmentation(self) -> float:
        """Calcula fragmentación del grafo de conocimiento"""
        if self.knowledge_graph.number_of_nodes() == 0:
            return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._calculate_graph_fragmentation").get("auto_param_L933_19", 0.0)
    ...
    def _calculate_contradiction_entropy(
        ...
        if not contradictions:
            return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._calculate_graph_fragmentation").get("auto_param_L950_19", 0.0)
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._calculate_syntactic_complexity")
    def _calculate_syntactic_complexity(self, text: str) -> float:
        ...
        # Combinar métricas
        complexity = (
                min(get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._calculate_syntactic_complexity").get("auto_param_L993_20", 1.0), avg_sentence_length / 50) * get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._calculate_syntactic_complexity").get("auto_param_L993_53", 0.3) +
                min(get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._calculate_syntactic_complexity").get("auto_param_L994_20", 1.0), avg_dependency_depth / 10) * get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._calculate_syntactic_complexity").get("auto_param_L994_54", 0.3) +
                ttr * get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._calculate_syntactic_complexity").get("auto_param_L995_22", 0.4)
        )

        return float(complexity)

    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._get_dependency_depth")
    ...
    def _calculate_confidence_interval(
        ...
        if n_observations < 30:
            ...
            t_critical = stats.t.ppf(get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._get_dependency_depth").get("auto_param_L1021_37", 0.975), n_observations - 1)
        ...
        return (
            max(get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._get_dependency_depth").get("auto_param_L1029_16", 0.0), score - margin),
            min(get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._get_dependency_depth").get("auto_param_L1030_16", 1.0), score + margin)
        )
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._get_graph_statistics")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._extract_temporal_markers")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._extract_quantitative_claims")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._parse_number")
    def _parse_number(self, text: str) -> float:
        ...
        try:
            ...
            return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._parse_number").get("auto_param_L1227_19", 0.0)

    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._extract_resource_mentions")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._determine_semantic_role")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._identify_dependencies")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._get_context_window")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._calculate_similarity")
    def _calculate_similarity(self, stmt_a: PolicyStatement, stmt_b: PolicyStatement) -> float:
        ...
        return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._calculate_similarity").get("auto_param_L1310_15", 0.0)

    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._classify_contradiction")
        ...
        try:
            ...
            for item in result:
                if 'contradiction' in item['label'].lower():
                    return item['score']
            return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._classify_contradiction").get("auto_param_L1321_19", 0.0)
        except Exception as e:
            logger.warning(f"Error en clasificación de contradicción: {e}")
            return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._classify_contradiction").get("auto_param_L1324_19", 0.0)

    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._get_domain_weight")
    def _get_domain_weight(self, dimension: PolicyDimension) -> float:
        """Obtiene peso específico del dominio"""
        weights = {
            PolicyDimension.DIAGNOSTICO: get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._get_domain_weight").get("auto_param_L1330_41", 0.8),
            PolicyDimension.ESTRATEGICO: 1.2,
            PolicyDimension.PROGRAMATICO: get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._get_domain_weight").get("auto_param_L1332_42", 1.0),
            PolicyDimension.FINANCIERO: 1.5,
            PolicyDimension.SEGUIMIENTO: get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._get_domain_weight").get("auto_param_L1334_41", 0.9),
            PolicyDimension.TERRITORIAL: 1.1
        }
        return weights.get(dimension, get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._get_domain_weight").get("auto_param_L1337_38", 1.0))

    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._suggest_resolutions")
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._are_comparable_claims")
    def _are_comparable_claims(self, claim_a: dict, claim_b: dict) -> bool:
        ...
        return context_similarity > get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._are_comparable_claims").get("auto_param_L1384_36", 0.6)

    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._text_similarity")
    def _text_similarity(self, text_a: str, text_b: str) -> float:
        """Calcula similaridad simple entre textos"""
        if not text_a or not text_b:
            return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._text_similarity").get("auto_param_L1390_19", 0.0)
        ...
        if not tokens_a or not tokens_b:
            return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._text_similarity").get("auto_param_L1397_19", 0.0)

        # Coeficiente de Jaccard
        intersection = tokens_a & tokens_b
        union = tokens_a | tokens_b

        return len(intersection) / len(union) if union else get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._text_similarity").get("auto_param_L1403_60", 0.0)
    ...
    def _statistical_significance_test(
        ...
        if pooled_value == 0:
            return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._text_similarity").get("auto_param_L1440_19", 1.0)  # No significativo

        # Estimación conservadora de error estándar
        se = pooled_value * get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._text_similarity").get("auto_param_L1443_28", 0.1)  # 10% de error estimado

        if se == 0:
            return get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._text_similarity").get("auto_param_L1446_19", 0.0)  # Altamente significativo
    ...
    @calibrated_method("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._has_logical_conflict")
    def _has_logical_conflict(self, stmt_a: PolicyStatement, stmt_b: PolicyStatement) -> bool:
        ...
        if has_negation_a != has_negation_b:
            ...
            if similarity > get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._has_logical_conflict").get("auto_param_L1481_28", 0.7):
                return True
    ...
    def _are_conflicting_allocations(
        ...
        return abs(amount_a - amount_b) / max(amount_a, amount_b) > get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._has_logical_conflict").get("auto_param_L1498_68", 0.5)
    ...
    def _calculate_severity(
        ...
        severity = get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._has_logical_conflict").get("severity", 0.5) # Refactored

        # Incrementar si las declaraciones están en la misma dimensión
        if stmt_a.dimension == stmt_b.dimension:
            severity += get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._has_logical_conflict").get("auto_param_L1532_24", 0.2)
        ...
        if len(common_entities) > 0:
            severity += min(get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._has_logical_conflict").get("auto_param_L1537_28", 0.2), len(common_entities) * get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._has_logical_conflict").get("auto_param_L1537_56", 0.05))

        # Incrementar si tienen marcadores temporales en conflicto
        if stmt_a.temporal_markers and stmt_b.temporal_markers:
            severity += get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._has_logical_conflict").get("auto_param_L1541_24", 0.1)

        return min(get_parameter_loader().get("farfan_pipeline.analysis.contradiction_deteccion.PolicyContradictionDetector._has_logical_conflict").get("auto_param_L1543_19", 1.0), severity)
