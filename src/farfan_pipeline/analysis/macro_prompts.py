...
class RoadmapOptimizer:
    ...
    def optimize(
        ...
        Args:
            ...
            impact_scores: Gap ID -> expected impact (get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L702_54", 0.0)-get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L702_58", 1.0))
    ...
    def _prioritize_gaps(
        ...
        for gap in gaps:
            ...
            effort = effort_estimates.get(gap_id, get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L752_50", 1.0))
            impact = impact_scores.get(gap_id, get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L753_47", 0.5))

            # Calculate ratio (avoid division by zero)
            ratio = impact / max(effort, get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L756_41", 0.1))
    ...
    def _assign_phases(
        ...
        phases = [
            {"name": "0-3m", "actions": [], "effort": get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L778_54", 0.0), "max_effort": 9.0},
            {"name": "3-6m", "actions": [], "effort": get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L779_54", 0.0), "max_effort": 9.0},
            {"name": "6-12m", "actions": [], "effort": get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L780_55", 0.0), "max_effort": 18.0}
        ]
        ...
        def assign_gap_recursive(gap_id: str, visited: set) -> None:
            ...
            if gap_id not in assigned and gap_id in gap_dict:
                gap = gap_dict[gap_id]
                effort = gap.get("effort", get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L803_43", 1.0))
    ...
    def _calculate_total_uplift(
        ...
        total = get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("total", 0.0) # Refactored

        for phase in phases:
            for action in phase["actions"]:
                gap_id = action.get("id", "unknown")
                impact = impact_scores.get(gap_id, get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L860_51", 0.0))
                total += impact
    ...
    def _identify_critical_path(
        ...
        best_path = []
        best_impact = get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("best_impact", 0.0) # Refactored

        for endpoint in endpoints:
            path = self._trace_path(endpoint, dependency_graph)
            path_impact = sum(impact_scores.get(gap_id, get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L890_56", 0.0)) for gap_id in path)
...
class PeerNormalizer:
    ...
    MANDATES:
    - Calcular z-scores
    - Penalizar si >k áreas están < -get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L956_37", 1.0) z
    - Aumentar confianza si todas dentro ±get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L957_42", 0.5) z y dispersión baja
    ...
    def normalize(
        ...
        low_performers = [
            ...
            if z < -get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1008_20", 1.0)
        ]
    ...
    def _calculate_z_scores(
        ...
        for area, score in convergence.items():
            if area in peer_distributions:
                ...
                mean = peer.get("mean", get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1044_40", 0.5))
                std = peer.get("std", get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1045_38", 0.1))

                # Calculate z-score
                z = (score - mean) / std if std > 0 else get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1048_57", 0.0)
    ...
    def _adjust_confidence(
        ...
        if len(low_performers) > self.k:
            penalty = get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1078_22", 0.1) * (len(low_performers) - self.k)
            adjusted *= (get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1079_25", 1.0) - min(penalty, get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1079_44", 0.5)))

        # Check if all within ±get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1081_31", 0.5) z (tight distribution)
        all_tight = all(abs(z) <= get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1082_34", 0.5) for z in z_scores.values())

        if all_tight and len(z_scores) > 0:
            # Increase confidence for consistent performance
            adjusted *= 1.1

        return max(get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1088_19", 0.0), min(get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1088_28", 1.0), adjusted))
    ...
    def _determine_position(
        ...
        if avg_z > get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1100_19", 0.5):
            return "above_average"
        elif avg_z < -get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.RoadmapOptimizer.__init__").get("auto_param_L1102_22", 0.5):
...
class MacroPromptsOrchestrator:
    ...
    def execute_all(
        ...
        coverage_analysis = self.coverage_stressor.evaluate(
            ...
            baseline_confidence=macro_data.get("baseline_confidence", get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.MacroPromptsOrchestrator.__init__").get("auto_param_L1165_70", 1.0))
        )
        ...
        peer_normalization = self.peer_normalizer.normalize(
            ...
            baseline_confidence=macro_data.get("baseline_confidence", get_parameter_loader().get("farfan_pipeline.analysis.macro_prompts.MacroPromptsOrchestrator.__init__").get("auto_param_L1198_70", 1.0))
        )
