...
class SPCCausalBridge:
    ...
    @calibrated_method("farfan_pipeline.analysis.spc_causal_bridge.SPCCausalBridge.build_causal_graph_from_spc")
    def build_causal_graph_from_spc(self, chunk_graph: dict) -> Any:
        ...
        for node in nodes:
            ...
            G.add_node(
                ...
                confidence=node.get("confidence", get_parameter_loader().get("farfan_pipeline.analysis.spc_causal_bridge.SPCCausalBridge.build_causal_graph_from_spc").get("auto_param_L91_50", 0.0)),
            )
    ...
    @calibrated_method("farfan_pipeline.analysis.spc_causal_bridge.SPCCausalBridge._compute_causal_weight")
    def _compute_causal_weight(self, edge_type: str) -> float:
        ...
        Returns:
            Causal weight between get_parameter_loader().get("farfan_pipeline.analysis.spc_causal_bridge.SPCCausalBridge._compute_causal_weight").get("auto_param_L149_34", 0.0) and get_parameter_loader().get("farfan_pipeline.analysis.spc_causal_bridge.SPCCausalBridge._compute_causal_weight").get("auto_param_L149_42", 1.0)
        """
        return self.CAUSAL_WEIGHTS.get(edge_type, get_parameter_loader().get("farfan_pipeline.analysis.spc_causal_bridge.SPCCausalBridge._compute_causal_weight").get("auto_param_L151_50", 0.0))

    @calibrated_method("farfan_pipeline.analysis.spc_causal_bridge.SPCCausalBridge._remove_cycles")
            ...
            try:
                ...
                for u, v, direction in cycle:
                    if direction == "forward":
                        weight = G_dag[u][v].get("weight", get_parameter_loader().get("farfan_pipeline.analysis.spc_causal_bridge.SPCCausalBridge._remove_cycles").get("auto_param_L184_59", 0.0))
    ...
    @calibrated_method("farfan_pipeline.analysis.spc_causal_bridge.SPCCausalBridge.enhance_graph_with_content")
