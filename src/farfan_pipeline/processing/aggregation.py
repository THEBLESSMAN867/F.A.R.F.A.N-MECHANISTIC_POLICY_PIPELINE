...
class DimensionAggregator:
    ...
    @calibrated_method("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_dimension_id")
    ...
    @calibrated_method("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_policy_area_id")
    ...
    @calibrated_method("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights")
    def validate_weights(self, weights: list[float]) -> tuple[bool, str]:
        """
        Ensures that a list of weights sums to get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L582_47", 1.0) within a small tolerance.
        ...
        if abs(weight_sum - get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L603_28", 1.0)) > tolerance:
            expected_weight = get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L604_81", 1.0)
    ...
    def calculate_weighted_average(
        ...
        """
        if not scores:
            return get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L665_19", 0.0)

        if weights is None:
            # Equal weights
            weights = [get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L669_23", 1.0) / len(scores)] * len(scores)
        ...
        # Validate weights sum to get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L679_34", 1.0)
    ...
    def apply_rubric_thresholds(
        ...
        clamped_score = max(get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L714_28", 0.0), min(3.0, score))
        ...
        if thresholds:
            excellent_threshold = thresholds.get('EXCELENTE', get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L721_62", 0.85))
            good_threshold = thresholds.get('BUENO', get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L722_53", 0.70))
            acceptable_threshold = thresholds.get('ACEPTABLE', get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L723_63", 0.55))
        else:
            excellent_threshold = get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("excellent_threshold", 0.85) # Refactored
            good_threshold = get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("good_threshold", 0.7) # Refactored
            acceptable_threshold = get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("acceptable_threshold", 0.55) # Refactored
    ...
    def aggregate_dimension(
        ...
        try:
            ...
            return DimensionScore(
                ...
                area_id=area_id,
                score=get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L795_22", 0.0),
        ...
        if not dim_results:
            ...
            return DimensionScore(
                dimension_id=dimension_id,
                area_id=area_id,
                score=get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L807_22", 0.0),
                quality_level="INSUFICIENTE",
                contributing_questions=[],
                validation_passed=False,
        ...
        try:
            ...
            return DimensionScore(
                dimension_id=dimension_id,
                area_id=area_id,
                score=get_parameter_loader().get("farfan_pipeline.processing.aggregation.DimensionAggregator.validate_weights").get("auto_param_L831_22", 0.0),
                quality_level="INSUFICIENTE",
    ...
    @calibrated_method("farfan_pipeline.processing.aggregation.DimensionAggregator._expected_question_count")
...
class AreaPolicyAggregator:
    ...
    @calibrated_method("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores")
    def normalize_scores(self, dimension_scores: list[DimensionScore]) -> list[float]:
        ...
        for d in dimension_scores:
            ...
            normalized.append(max(get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("auto_param_L1148_34", 0.0), min(max_expected, d.score)) / max_expected)
    ...
    def apply_rubric_thresholds(
        ...
        """
        # Clamp score to valid range [0, 3]
        clamped_score = max(get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("auto_param_L1170_28", 0.0), min(3.0, score))
        ...
        # Use provided thresholds or defaults
        if thresholds:
            excellent_threshold = thresholds.get('EXCELENTE', get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("auto_param_L1177_62", 0.85))
            good_threshold = thresholds.get('BUENO', get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("auto_param_L1178_53", 0.70))
            acceptable_threshold = thresholds.get('ACEPTABLE', get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("auto_param_L1179_63", 0.55))
        else:
            excellent_threshold = get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("excellent_threshold", 0.85) # Refactored
            good_threshold = get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("good_threshold", 0.7) # Refactored
            acceptable_threshold = get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("acceptable_threshold", 0.55) # Refactored
    ...
    def aggregate_area(
        ...
        try:
            ...
            return AreaScore(
                area_id=area_id,
                area_name=area_name,
                score=get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("auto_param_L1249_22", 0.0),
                quality_level="INSUFICIENTE",
                dimension_scores=[],
                validation_passed=False,
        ...
        if not area_dim_scores:
            ...
            return AreaScore(
                area_id=area_id,
                area_name=area_name,
                score=get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("auto_param_L1266_22", 0.0),
                quality_level="INSUFICIENTE",
                dimension_scores=[],
                validation_passed=False,
...
class ClusterAggregator:
    ...
    PENALTY_WEIGHT = get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("PENALTY_WEIGHT", 0.3) # Refactored
    MAX_SCORE = 3.0
    ...
    def apply_cluster_weights(
        ...
        if weights is None:
            # Equal weights
            weights = [get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("auto_param_L1495_23", 1.0) / len(scores)] * len(scores)
        ...
        # Validate weights sum to get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("auto_param_L1507_34", 1.0)
        weight_sum = sum(weights)
        tolerance = 1e-6
        if abs(weight_sum - get_parameter_loader().get("farfan_pipeline.processing.aggregation.AreaPolicyAggregator.normalize_scores").get("auto_param_L1510_28", 1.0)) > tolerance:
    ...
    @calibrated_method("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence")
    def analyze_coherence(self, area_scores: list[AreaScore]) -> float:
        ...
        if len(scores) <= 1:
            return get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1543_19", 1.0)
        ...
        std_dev = variance ** get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1550_30", 0.5)

        # Convert to coherence (inverse relationship)
        # Normalize by max possible std dev (3.0 for 0-3 range)
        max_std = 3.0
        coherence = max(get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1555_24", 0.0), get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1555_29", 1.0) - (std_dev / max_std))
    ...
    def aggregate_cluster(
        ...
        if not cluster_def:
            ...
            return ClusterScore(
                ...
                areas=[],
                score=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1600_22", 0.0),
                coherence=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1601_26", 0.0),
                variance=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1602_25", 0.0),
                weakest_area=None,
                area_scores=[],
                validation_passed=False,
        ...
        try:
            ...
            return ClusterScore(
                ...
                score=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1631_22", 0.0),
                coherence=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1632_26", 0.0),
                variance=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1633_25", 0.0),
                weakest_area=None,
                area_scores=[],
                validation_passed=False,
                validation_details={"error": str(e), "type": "hermeticity"}
        ...
        if not cluster_area_scores:
            ...
            return ClusterScore(
                ...
                cluster_name=cluster_name,
                areas=expected_areas,
                score=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1646_22", 0.0),
                coherence=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1647_26", 0.0),
                variance=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1648_25", 0.0),
                weakest_area=None,
                area_scores=[],
                validation_passed=False,
        ...
        try:
            ...
            return ClusterScore(
                ...
                cluster_name=cluster_name,
                areas=expected_areas,
                score=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1670_22", 0.0),
                coherence=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1671_26", 0.0),
                variance=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1672_25", 0.0),
                weakest_area=None,
        ...
        if scores_array:
            ...
            variance = get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("variance", 0.0) # Refactored
        weakest_area = min(cluster_area_scores, key=lambda a: a.score, default=None)

        std_dev = variance ** get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1689_30", 0.5)
        normalized_std = min(std_dev / self.MAX_SCORE, get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1690_55", 1.0)) if std_dev > 0 else get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1690_80", 0.0)
        penalty_factor = get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1691_25", 1.0) - (normalized_std * self.PENALTY_WEIGHT)
        adjusted_score = weighted_score * penalty_factor

        validation_details["coherence"] = {
            "value": coherence,
            "interpretation": "high" if coherence > get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1696_52", 0.8) else "medium" if coherence > get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1696_85", 0.6) else "low"
        }
...
class MacroAggregator:
    ...
    def calculate_cross_cutting_coherence(
        ...
        if len(scores) <= 1:
            return get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1847_19", 1.0)
        ...
        # Calculate standard deviation
        variance = sum((s - mean) ** 2 for s in scores) / len(scores)
        std_dev = variance ** get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1854_30", 0.5)

        # Convert to coherence
        max_std = 3.0
        coherence = max(get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1858_24", 0.0), get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1858_29", 1.0) - (std_dev / max_std))
    ...
    def assess_strategic_alignment(
        ...
        cluster_coherence = (
            ...
            if cluster_scores else get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1907_35", 0.0)
        )

        # Calculate dimension validation rate
        validated_dims = sum(1 for d in dimension_scores if d.validation_passed)
        validation_rate = validated_dims / len(dimension_scores) if dimension_scores else get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1912_90", 0.0)

        # Strategic alignment is weighted combination
        alignment = (get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1915_21", 0.6) * cluster_coherence) + (get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1915_49", 0.4) * validation_rate)
    ...
    def apply_rubric_thresholds(
        ...
        """
        # Clamp score to valid range [0, 3]
        clamped_score = max(get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1941_28", 0.0), min(3.0, score))
        ...
        # Use provided thresholds or defaults
        if thresholds:
            excellent_threshold = thresholds.get('EXCELENTE', get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1948_62", 0.85))
            good_threshold = thresholds.get('BUENO', get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1949_53", 0.70))
            acceptable_threshold = thresholds.get('ACEPTABLE', get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1950_63", 0.55))
        else:
            excellent_threshold = get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("excellent_threshold", 0.85) # Refactored
            good_threshold = get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("good_threshold", 0.7) # Refactored
            acceptable_threshold = get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("acceptable_threshold", 0.55) # Refactored
    ...
    def evaluate_macro(
        ...
        if not cluster_scores:
            ...
            return MacroScore(
                score=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1997_22", 0.0),
                quality_level="INSUFICIENTE",
                cross_cutting_coherence=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L1999_40", 0.0),
                systemic_gaps=[],
                strategic_alignment=get_parameter_loader().get("farfan_pipeline.processing.aggregation.ClusterAggregator.analyze_coherence").get("auto_param_L2001_36", 0.0),
                cluster_scores=[],
                validation_passed=False,
    ...
    @calibrated_method("farfan_pipeline.processing.aggregation.MacroAggregator._calculate_macro_score")
    def _calculate_macro_score(self, cluster_scores: list[ClusterScore]) -> float:
        ...
        if not cluster_scores:
            return get_parameter_loader().get("farfan_pipeline.processing.aggregation.MacroAggregator._calculate_macro_score").get("auto_param_L2061_19", 0.0)
        if not weights:
