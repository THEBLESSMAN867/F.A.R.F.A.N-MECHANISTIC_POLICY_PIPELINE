...
- Repo root MUST contain farfan_core/farfan_pipeline.
...
def compute_repo_root() -> Path:
    ...
    Strategy:
    - Walk parents of this file until we find one that contains farfan_core/farfan_pipeline.
    - Fallback to farfan_pipeline.config.paths.PROJECT_ROOT if available.
    ...
    for candidate in here.parents:
        if (candidate / "farfan_pipeline" / "farfan_pipeline").exists():
            return candidate
    ...
    try:
        ...
        if (project_root / "farfan_pipeline" / "farfan_pipeline").exists():
            return project_root
    except Exception:
        pass
...
def _load_third_party_from_lockdown(repo_root: Path) -> FrozenSet[str]:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "farfan_pipeline.config.dependency_lockdown. This module MUST exist "
...
def build_import_policy(repo_root: Path) -> ImportPolicy:
    ...
    Maximum hardness:
    - Internal imports allowed only under 'farfan_pipeline.*'.
    ...
    allowed_internal: FrozenSet[str] = frozenset(
        {
            ...
            "farfan_pipeline.entrypoint",
            "farfan_pipeline.scripts",
            "farfan_pipeline.analysis",
            "farfan_pipeline.audit",
            "farfan_pipeline.observability",
            "farfan_pipeline.utils",
            "farfan_pipeline.scoring",
            "farfan_pipeline.processing",
            "farfan_pipeline.patterns",
            "farfan_pipeline.infrastructure",
            "farfan_pipeline.flux",
            "farfan_pipeline.config",
            "farfan_pipeline.controls",
            "farfan_pipeline.concurrency",
            "farfan_pipeline.optimization",
            "farfan_pipeline.api",
            "farfan_pipeline",
        }
    )
