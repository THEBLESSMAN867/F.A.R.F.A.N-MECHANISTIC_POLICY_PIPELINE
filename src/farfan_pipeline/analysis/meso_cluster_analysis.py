...
class MetricViolation:
    ...
    @calibrated_method("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict")
...
def reconcile_cross_metrics(
    ...
    for metric in aggregated_metrics:
        ...
        value = float(metric.get("value", get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L246_42", 0.0)))
    ...
    reconciled_confidence = max(get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L306_32", 0.0), get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L306_37", 1.0) - total_violations / total_checks)
...
def compose_cluster_posterior(
    ...
    if weighting_trace is None:
        weights = [get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L326_19", 1.0)] * len(posts)
    else:
    ...
    if all(w == 0 for w in weights):
        weights = [get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L334_19", 1.0)] * len(posts)

    # Prevent degenerate/negative totals; fallback to uniform if needed.
    weights = [max(get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L337_19", 0.0), float(w)) for w in weights]
    ...
    uncertainty_index = float(variance ** get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L345_42", 0.5))

    penalties_input = reconciliation_penalties or {}
    dispersion_penalty = float(penalties_input.get("dispersion_penalty", get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L348_73", 0.0)))
    coverage_penalty = float(penalties_input.get("coverage_penalty", get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L349_69", 0.0)))
    reconciliation_penalty = float(penalties_input.get("reconciliation_penalty", get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L350_81", 0.0)))

    penalty_factor = reduce(
        lambda acc, val: acc * val,
        [
            max(get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L355_16", 0.0), get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L355_21", 1.0) - dispersion_penalty),
            max(get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L356_16", 0.0), get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L356_21", 1.0) - coverage_penalty),
            max(get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L357_16", 0.0), get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L357_21", 1.0) - reconciliation_penalty),
        ],
        get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L359_8", 1.0),
    )
...
def calibrate_against_peers(
    ...
    if dispersion_values:
        ...
        cluster_cv = cluster_std / cluster_mean if cluster_mean else get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("auto_param_L395_69", 0.0)
    else:
        cluster_cv = get_parameter_loader().get("farfan_pipeline.analysis.meso_cluster_analysis.MetricViolation.to_flag_dict").get("cluster_cv", 0.0) # Refactored
