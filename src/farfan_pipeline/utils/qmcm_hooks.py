...
class QMCMRecorder:
    ...
    def record_call(
            ...
            execution_time_ms: float = get_parameter_loader().get("farfan_pipeline.utils.qmcm_hooks.QMCMRecorder.__init__").get("auto_param_L45_39", 0.0),
    ...
    @calibrated_method("farfan_pipeline.utils.qmcm_hooks.QMCMRecorder.get_statistics")
    def get_statistics(self) -> dict[str, Any]:
        ...
        if not self.calls:
            return {
                ...
                "unique_methods": 0,
                "method_frequency": {},
                "success_rate": get_parameter_loader().get("farfan_pipeline.utils.qmcm_hooks.QMCMRecorder.get_statistics").get("auto_param_L94_32", 0.0),
        ...
        return {
            ...
            "success_rate": success_count / len(self.calls) if self.calls else get_parameter_loader().get("farfan_pipeline.utils.qmcm_hooks.QMCMRecorder.get_statistics").get("auto_param_L116_79", 0.0),
            "most_called_method": most_called
        }

    @calibrated_method("farfan_pipeline.utils.qmcm_hooks.QMCMRecorder.save_recording")
    ...
    @calibrated_method("farfan_pipeline.utils.qmcm_hooks.QMCMRecorder.load_recording")
    ...
    @calibrated_method("farfan_pipeline.utils.qmcm_hooks.QMCMRecorder.clear_recording")
    def clear_recording(self) -> None:
        """Clear all recorded calls"""
        self.calls = []
        logger.info("QMCM recording cleared")

    @calibrated_method("farfan_pipeline.utils.qmcm_hooks.QMCMRecorder.enable")
    def enable(self) -> None:
        """Enable recording"""
        self.enabled = True

    @calibrated_method("farfan_pipeline.utils.qmcm_hooks.QMCMRecorder.disable")
...
def qmcm_record(method=None, *, monolith_hash: str | None = None):
    ...
    Usage:
        @qmcm_record
        @calibrated_method("farfan_pipeline.utils.qmcm_hooks.QMCMRecorder.my_method")
        ...
        @calibrated_method("farfan_pipeline.utils.qmcm_hooks.QMCMRecorder.my_questionnaire_method")
